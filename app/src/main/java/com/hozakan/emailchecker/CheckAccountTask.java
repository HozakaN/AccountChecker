...
import com.hozakan.emailchecker.data.AccountCheckerProvider;
...
public class CheckAccountTask extends AsyncTask<String, Void, Void> {
    ...
    protected Void doInBackground(String... params) {
        ...
        try {
            ...
            urlConnection.setRequestProperty("X-Mashape-Key", "Enter your key here");
...
//            JSONObject forecastJson = new JSONObject(forecastJsonStr);
            ...
            for (int i = 0; i < pwndArray.length(); i++) {
                ...
                contentValues.put(PwndEntry.COLUMN_ACCOUNT_ID, accountId);
                contentValues.put(PwndEntry.COLUMN_TITLE, pwndData.getString(TITLE));
                contentValues.put(PwndEntry.COLUMN_NAME, pwndData.getString(NAME));
                contentValues.put(PwndEntry.COLUMN_DOMAIN, pwndData.getString(DOMAIN));
                contentValues.put(PwndEntry.COLUMN_BREACH_DATE, pwndData.getString(BREACH_DATE));
                contentValues.put(PwndEntry.COLUMN_ADDED_DATE, pwndData.getString(ADDED_DATE));
                contentValues.put(PwndEntry.COLUMN_DESCRIPTION, pwndData.getString(DESCRIPTION));
                contentValues.put(PwndEntry.COLUMN_PWND_COUNT, pwndData.getInt(PWN_COUNT));

                Uri uri = mContext.getContentResolver().insert(PwndContract.CONTENT_URI, contentValues);
                String pwdId = PwndContract.getIdFromUri(uri);

                Vector<ContentValues> cVVector = new Vector<>(dataClassIds.size());

                for (String dataClassId : dataClassIds) {
                    contentValues = new ContentValues();
                    contentValues.put(DataClassToPwndEntry.COLUMN_DATACLASS_ID, dataClassId);
                    contentValues.put(DataClassToPwndEntry.COLUMN_PWND_ID, pwdId);
                    cVVector.add(contentValues);
                }

                mContext.getContentResolver()
                        .bulkInsert(DataClassToPwndContract.CONTENT_URI, cVVector.toArray(new ContentValues[]{}));
...
////            return resultStrs;
    ...
    private void deletePwndDataForAccount(String accountId) {
        mContext.getContentResolver()
                .delete(
                    PwndContract.CONTENT_URI,
                    AccountCheckerProvider.sPwndForAccountIdSelection,
